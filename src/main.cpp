...
const std::string VERSION = "GreKo Junior 12.9";
...
private:
	...
	void OnList()
	{
		...
		{
			out("%s ", MoveToStrLong(mvlist[i].m_mv).c_str());
		}
		out(" -- total %d moves\n", mvlist.Size());
	}
	...
	void OnProtover()
	{
		out("\nfeature myname=\"%s\"", VERSION.c_str());
	...
	void OnUCI()
	{
		g_protocol = UCI;
		out("id name %s\n", VERSION.c_str());
		out("id author Sergey Kozlov, Vladimir Medvedev\n");
		...
		out("option name PawnSpace type spin default %d min 0 max 100\n", DEFAULT_PAWN_SPACE);
	...
	void OnSetoption()
	{
		...
		else if (CaseInsensitiveEquals(key, "PawnSpace"))
			g_evalParams.PawnSpace = atoi(value.c_str());
...
#define ON_CMD(pattern, minLen, action)    \
	...
	void Init()
	{
		Config config("GreKo-Junior.ini");
		if (config.GetInt("WriteLog", 0)) g_log = fopen("greko-junior.log", "at");
		...
		g_evalParams.PawnSpace = config.GetInt("PawnSpace", DEFAULT_PAWN_SPACE);
