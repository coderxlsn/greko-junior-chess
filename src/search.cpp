...
EVAL Search::AlphaBetaRoot(EVAL alpha, EVAL beta, const int depth)
{
	...
	for (int i = 0; i < mvlist.Size(); ++i)
	{
		...
		if (m_pos.MakeMove(mv))
		{
			...
			{
				out("info currmove %s", MoveToStrLong(mv).c_str());
...
void Search::PrintPV()
{
	...
	for (size_t m = 0; m < m_rootPV.Size(); ++m)
	{
		...
		out("%s", MoveToStrShort(mv, tmp).c_str());
		tmp.MakeMove(mv);
...
MAKE_MOVE:
	...
	if (m_flag)
	{
		if (g_protocol == UCI)
		{
//			out("\nbestmove %s\n", MoveToStrLong(best_move).c_str());
			out("\nbestmove %s ponder %s\n", MoveToStrLong(best_move).c_str(), MoveToStrLong(m_rootPV[1]).c_str());
		}
		else
		{
			Highlight(true);
